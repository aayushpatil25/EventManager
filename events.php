...
// Fetch services for dropdown (Main category only)
$service_sql = "SELECT * FROM services_category WHERE category = 'Main' AND status = 'Active' ORDER BY name ASC";
$service_result = mysqli_query($conn, $service_sql);

// Fetch requirements from services_category where category != 'Main', grouped by parent service name
$requirements_sql = "SELECT * FROM services_category WHERE category != 'Main' AND status = 'Active' ORDER BY category, name ASC";
...
while ($req = mysqli_fetch_assoc($requirements_result)) {
    $requirementsByService[$req['category']][] = $req['name'];
}

// Fetch all events
$events_sql = "SELECT e.*, c.contact,c.client_name
               FROM events e 
...
// Fetch past events
$past_events_query = "SELECT e.*, c.contact 
                      FROM events e 
                      LEFT JOIN clients c ON e.client_name = c.client_name 
                      WHERE DATE(e.end_date) < CURDATE() 
...
<style>
    ...
    @media print {
        ...
        body.printing-table .sidebar,
        body.printing-table .topbar,
        body.printing-table .page-header,
        body.printing-table .modal-overlay,
        body.printing-table .search-bar,
        body.printing-table .no-print{
            display: none !important;
        }

        body.printing-table .clients-table th,
        body.printing-table .clients-table td {
            border: 1px solid #000;
            padding: 8px;
            font-size: 12px;
        }

            body.printing-table .clients-table thead {
            background: #f3f4f6 !important;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        }

        body.printing-table .clients-table thead th {
            color: #000 !important;
            background: #f3f4f6 !important;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        ...
        body.printing-table .table-container {
    ...
    .catering-toggle-option input[type="radio"] {
        display: none;
    }

    .catering-toggle-pill {
        display: inline-flex;
        align-items: center;
        gap: 6px;
        padding: 7px 14px;
    ...
    /* Selected states */
    input[type="radio"].catering-radio:checked+.catering-toggle-pill {
        background: white;
    }

    .catering-veg:checked+.catering-toggle-pill {
        border-color: #16a34a;
        background: #f0fdf4;
        color: #15803d;
    }

    .catering-nonveg:checked+.catering-toggle-pill {
        border-color: #dc2626;
        background: #fef2f2;
        color: #b91c1c;
    }

    .catering-both:checked+.catering-toggle-pill {
        border-color: #0ea5e9;
        background: #f0f9ff;
        color: #0369a1;
    }

    /* Catering type badge in view modal */
    .catering-type-badge {
        display: inline-flex;
        align-items: center;
        gap: 6px;
    ...
    .catering-type-badge.veg {
        background: #f0fdf4;
        color: #15803d;
        border: 1px solid #bbf7d0;
    }

    .catering-type-badge.nonveg {
        background: #fef2f2;
        color: #b91c1c;
        border: 1px solid #fecaca;
    }

    .catering-type-badge.both {
        background: #f0f9ff;
        color: #0369a1;
    ...
    .catering-time-group input[type="time"] {
        padding: 8px 10px;
        border: 1px solid #e2e8f0;
        border-radius: 6px;
        font-size: 13px;
    ...
    .catering-time-group input[type="time"]:focus {
        outline: none;
        border-color: #0ea5e9;
        background: white;
        box-shadow: 0 0 0 3px rgba(14, 165, 233, 0.1);
    ...
    /* Past Events Modal Table */
    #pastEventsModal .modal-content {
        max-width: 1000px;
    }

    #pastEventsModal .modal-body {
        padding: 20px;
    ...
    #pastEventsModal table {
        width: 100%;
        border-collapse: collapse;
        font-size: 14px;
    }

    #pastEventsModal thead tr {
        background: #f8fafc;
        position: sticky;
        top: 0;
        z-index: 1;
    ...
    #pastEventsModal tbody tr {
        transition: background 0.15s ease;
    }

    #pastEventsModal tbody tr:hover {
        background: #f8fafc;
    }

    #pastEventsModal tbody tr:last-child td {
        border-bottom: none;
    }

    /* Serial number */
    #pastEventsModal .col-sr {
        color: #94a3b8;
        font-size: 13px;
        width: 40px;
    }
    ...
    /* Event name */
    #pastEventsModal .col-event {
        font-weight: 600;
        color: #1e293b;
    }

    /* Client badge */
    #pastEventsModal .badge-client {
        display: inline-flex;
        align-items: center;
        background: #ede9fe;
        color: #6b21a8;
    ...
    /* Venue */
    #pastEventsModal .col-venue {
        color: #64748b;
        font-size: 13px;
    }

    /* Date chips */
    #pastEventsModal .badge-date {
        display: inline-flex;
        align-items: center;
        gap: 5px;
        padding: 4px 12px;
    ...
    #pastEventsModal .badge-date.start {
        background: #dbeafe;
        color: #1e40af;
    }

    #pastEventsModal .badge-date.end {
        background: #fee2e2;
        color: #dc2626;
    }

    /* Budget */
    #pastEventsModal .col-budget {
        font-weight: 700;
        color: #10b981;
        white-space: nowrap;
    }
    ...
    /* Empty state */
    #pastEventsModal .empty-state {
        padding: 80px 20px;
        text-align: center;
        color: #94a3b8;
    }

    #pastEventsModal .empty-state i {
        font-size: 52px;
        margin-bottom: 16px;
        opacity: 0.25;
        display: block;
    ...
    body.printing-past #pastEventsModal,
    body.printing-past #pastEventsModal * {
        visibility: visible;
    }
    ...
    body.printing-past #pastEventsModal .modal {
        max-width: 100%;
        width: 100%;
        max-height: none;
        box-shadow: none !important;
    ...
    body.printing-past #pastEventsModal .modal-header,
    body.printing-past #pastEventsModal .filtersBtn {
        display: none !important;
    }

    body.printing-past #pastEventsModal table th,
    body.printing-past #pastEventsModal table td {
        border: 1px solid #000 !important;
        font-size: 11px !important;
        padding: 8px !important;
    }
    body.printing-past #pastEventsModal .no-print {
    display: none !important;
}
}
...
.btn-past-export:hover {
    background: #dcfce7 !important;
    transform: translateY(-2px);
}

.btn-past-print:hover {
    background: #dbeafe !important;
    transform: translateY(-2px);
}
...
<main class="main-content">
    <div class="clients-container">
        <div class="page-header">
            ...
            <button class="btn-add-client" onclick="pastEventModal()" style="background-color: #e1ebff;color: #090045;border-color: #cbd5e1;">
                Past Events
            </button>
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($event['venue']); ?>
                                </td>
                                <td>
                                    <span class="client-badge"><?php echo htmlspecialchars($event['client_name']); ?></span>
                                </td>
                                <td>
                                    <span class="contact-badge"><?php echo htmlspecialchars($event['contact']); ?></span>
                                </td>
                                <td>
                                    <?php
                                    $req_total = 0;
                                    if (!empty($event['service'])) {
                                        $svc_data = json_decode($event['service'], true);
                                            ...
                                            foreach ($svc_data as $svc) {
                                                if (!empty($svc['requirements']) && is_array($svc['requirements'])) {
                                                    foreach ($svc['requirements'] as $req) {
                                                        $req_total += floatval($req['amount'] ?? 0);
                                                    }
                                                }
                                            }
                                        }
                                    }
                                    echo '₹' . number_format($req_total, 2);
                                    ?>
                                </td>
                                <td class="no-print">
                                    <div class="action-buttons no-print">
                                        ...
                                        <a href="events.php?edit=<?php echo $event['id']; ?>" class="btn-action edit no-print">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <table class="services-table full-width">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width: 120px;">Service Name</th>
                                            <th>Requirements</th>
                        ...
                        <table class="services-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 78%;">Requirements</th>
                                </tr>
                            </thead>
                    ...
                    <div class="services-table-container">
                        <table class="services-table">
                            <thead>
                                <tr>
                                    <th style="width: 60px;">Sr</th>
                                    <th style="width: 130px;">Service Name</th>
                                    <th>Requirements</th>
                                    <th style="width: 100px;">Action</th>
                                </tr>
                            </thead>
...
<!-- Past Events Modal -->
<div class="modal-overlay" id="pastEventsModal">
    <div class="modal">
        <div class="modal-header">
            <h2 class="modal-title">Past Events</h2>
            <button class="modal-close" onclick="closePastEventsModal()">
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path fill="currentColor" d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" />
                </svg>
            </button>
        ...
        <div class="modal-body">
            ...
            <div class="filtersBtn">
                <div class="past-search-bar">
                    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path fill="currentColor" d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" />
                    </svg>
                    <input type="text" id="pastSearchInput" placeholder="Search past events...">
                </div>
                <button class="btn-past-export" onclick="exportExcelPast()" style="background: #f0fdf4;
                ...
                <button class="btn-past-print" onclick="printTablePast()" style="background: #eff6ff;
                color: #1d4ed8;border-color: #bfdbfe;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="6 9 6 2 18 2 18 9" />
                        <path d="M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2" />
                ...
                <table style="width:100%; border-collapse:collapse; font-size:14px;">
                    <thead>
                        <tr style="background:#f8fafc;">
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">Sr.</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">Event</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">Client</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">Client Contact</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">Venue</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">End Date</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;">Amount</th>
                            <th style="padding:14px 18px; text-align:left; color:#64748b; font-size:11px; font-weight:700; text-transform:uppercase; border-bottom:2px solid #e2e8f0;" class="no-print">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($past_events as $i => $ev): ?>
                            <tr style="border-bottom:1px solid #f1f5f9;"
                                onmouseover="this.style.background='#f8fafc'"
                                onmouseout="this.style.background='white'">
                                <td style="padding:14px 18px; color:#94a3b8;"><?= $i + 1 ?></td>
                                <td style="padding:14px 18px; font-weight:600; color:#1e293b;"><?= htmlspecialchars($ev['name']) ?></td>
                                <td style="padding:14px 18px;">
                                    <span style="background:#ede9fe; color:#6b21a8; padding:4px 12px; border-radius:20px; font-size:12px; font-weight:600;">
                                        <?= htmlspecialchars($ev['client_name']) ?>
                                    </span>
                                </td>
                                <td style="padding:14px 18px; font-weight:600; color:#1e293b;"><?= htmlspecialchars($ev['contact']) ?></td>
                                <td style="padding:14px 18px; color:#64748b;"><?= htmlspecialchars($ev['venue']) ?></td>
                                <td style="padding:14px 18px;">
                                    <span style="background:#fee2e2; color:#dc2626; padding:4px 12px; border-radius:20px; font-size:12px; font-weight:600;">
                                        <?= date('d M Y', strtotime($ev['end_date'])) ?>
                                    </span>
                                </td>
                                <td style="padding:14px 18px; font-weight:700; color:#10b981;">
                                    ₹<?= number_format($ev['budget']) ?>
                                </td>
                                <td style="padding:14px 18px;">
                                    <div style="display:flex; gap:8px;">
                                        <button onclick='viewDetails(<?= json_encode($ev) ?>)' class="btn-action view" title="View">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                                <circle cx="12" cy="12" r="3"/>
                                            </svg>
                                        </button>
                                        <a href="events.php?edit=<?= $ev['id'] ?>" class="btn-action edit" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
                                                <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
                                            </svg>
...
<script>
    ...
    // Requirements from DB, keyed by service name
    const allRequirements = <?= json_encode($requirementsByService) ?>;

    function getRequirementsForService(serviceName) {
        return (allRequirements[serviceName] && allRequirements[serviceName].length > 0) ?
            allRequirements[serviceName] :
            staticRequirementOptions;
    }

    // Static requirements options (fallback if no subcategories exist)
    const staticRequirementOptions = [
        'Veg',
        'Non-Veg',
        'Beverages',
        'Desserts',
    ...
    function generateRequirementDropdown(selectedValue = '', options = staticRequirementOptions, usedValues = []) {
        let html = '<option value="">--Select Requirement--</option>';
        options.forEach(option => {
            // Show if it's the current row's selected value OR not used by another row
            if (option === selectedValue || !usedValues.includes(option)) {
    ...
    function addServiceToTable() {
        ...
        const subcategories = getRequirementsForService(selectedValue);
    ...
    function updateRequirementDescription(serviceIndex, reqIndex, value) {
        selectedServices[serviceIndex].requirements[reqIndex].description = value;
        updateHiddenInput();
    }
    ...
    // OLD: function generateRequirementsHTML(serviceIndex, requirements) {
    // Replace the whole function with:

    function isCatering(serviceName) {
        return serviceName.trim().toLowerCase() === 'catering';
    }

    function generateCateringToggleHTML(serviceIndex, currentValue, prefix = '') {
        const name = `${prefix}catering_type_${serviceIndex}`;
        const fn = prefix === 'edit_' ?
            `updateEditCateringType(${serviceIndex}, this.value)` :
            `updateCateringType(${serviceIndex}, this.value)`;
        const opts = [{
                value: 'Veg',
                cls: 'veg',
                dotCls: 'veg',
        ...
        return `
        <div class="catering-toggle-group">
            ${opts.map(o => `
                <label class="catering-toggle-option">
                    <input type="radio"
                           class="catering-radio catering-${o.cls}"
                           name="${name}"
                           value="${o.value}"
                           ${currentValue === o.value ? 'checked' : ''}
                           onchange="${fn}">
                    <span class="catering-toggle-pill">
                        <span class="catering-dot ${o.dotCls}"></span>
                        ${o.label}
                    </span>
                </label>
            `).join('')}
    ...
    const options = service.subcategories || staticRequirementOptions;
    // Collect all selected types in this service (for filtering)
    const usedTypes = requirements.map(r => r.type).filter(t => t !== '');

    if (isCatering(service.service)) {
        const currentVal = service.cateringType || '';

        return `
        <div style="display:flex; flex-direction:column; gap:12px;">

            <div class="catering-section-divider">Meal Type</div>
            ${generateCateringToggleHTML(serviceIndex, currentVal, '')}

            <div class="catering-section-divider">Items / Subcategories</div>
            <div class="requirements-container">
                ${requirements.map((req, reqIndex) => {
                    const otherUsed = usedTypes.filter(t => t !== req.type);
                    return `
                    <div class="requirement-row" style="flex-wrap:wrap; gap:6px; align-items:center;">
                        <select onchange="updateRequirementType(${serviceIndex}, ${reqIndex}, this.value)" style="min-width:130px; flex:1;">
                            ${generateRequirementDropdown(req.type, options, otherUsed)}
                        </select>
                        <input type="text"
                               value="${req.description || ''}"
                               onchange="updateRequirementDescription(${serviceIndex}, ${reqIndex}, this.value)"
                               style="flex:2; min-width:120px;" placeholder="Description">
                        <input type="number"
                               value="${req.amount || ''}"
                               onchange="updateRequirementAmount(${serviceIndex}, ${reqIndex}, this.value)"
                               min="0" style="max-width:100px;" placeholder="₹ Amount">
                        <div style="display:flex; align-items:center; gap:4px;">
                            <input type="time"
                                   value="${req.startTime || ''}"
                                   onchange="updateRequirementTime(${serviceIndex}, ${reqIndex}, 'start', this.value)"
                                   style="width:120px;padding:5px 14px">
                        </div>
                        <div style="display:flex; align-items:center; gap:4px;">
                            <span style="font-size:11px; color:#64748b; white-space:nowrap;">To</span>
                            <input type="time"
                                   value="${req.endTime || ''}"
                                   onchange="updateRequirementTime(${serviceIndex}, ${reqIndex}, 'end', this.value)"
                                   style="width:120px;padding:5px 14px">
                        </div>
            ...
            ${requirements.map((req, reqIndex) => {
                ...
                <div class="requirement-row">
                    ...
                    <input type="text"
                           value="${req.description || ''}"
                           onchange="updateRequirementDescription(${serviceIndex}, ${reqIndex}, this.value)"
                           style="flex:2;" placeholder="Description">
                    <input type="number"
                           value="${req.amount || ''}"
                           onchange="updateRequirementAmount(${serviceIndex}, ${reqIndex}, this.value)"
                           min="0" style="max-width:100px;" placeholder="₹ Amount">
   ...
   function printTable() {
    ...
    setTimeout(() => {
        ...
        document.getElementById('print-table-header')?.remove();
    }, 100);
}
    ...
    // Update edit requirement description
    function updateEditRequirementDescription(serviceIndex, reqIndex, value) {
        editSelectedServices[serviceIndex].requirements[reqIndex].description = value;
        updateEditHiddenInput();
    }
    ...
    const service = editSelectedServices[serviceIndex];
    const options = service.subcategories || staticRequirementOptions;
    const usedTypes = requirements.map(r => r.type).filter(t => t !== '');
    ...
    if (isCatering(service.service)) {
        ...
        <div style="display:flex; flex-direction:column; gap:12px;">
            ...
            <div class="catering-section-divider">Meal Type</div>
            ${generateCateringToggleHTML(serviceIndex, currentVal, 'edit_')}
            ...
            <div class="requirements-container">
                ${requirements.map((req, reqIndex) => {
                    ...
                    return `
                    <div class="requirement-row" style="flex-wrap:wrap; gap:6px; align-items:center;">
                        <select onchange="updateEditRequirementType(${serviceIndex}, ${reqIndex}, this.value)" style="min-width:130px; flex:1;">
                            ${generateRequirementDropdown(req.type, options, otherUsed)}
                        </select>
                        <input type="text"
                               value="${req.description || ''}"
                               onchange="updateEditRequirementDescription(${serviceIndex}, ${reqIndex}, this.value)"
                               style="flex:2; min-width:120px;" placeholder="Description">
                        <input type="number"
                               value="${req.amount || ''}"
                               onchange="updateEditRequirementAmount(${serviceIndex}, ${reqIndex}, this.value)"
                               min="0" style="max-width:100px;" placeholder="₹ Amount">
                        <div style="display:flex; align-items:center; gap:4px;">
                            <span style="font-size:11px; color:#64748b; white-space:nowrap;">From</span>
                            <input type="time"
                                   value="${req.startTime || ''}"
                                   onchange="updateEditRequirementTime(${serviceIndex}, ${reqIndex}, 'start', this.value)"
                                   style="width:120px;">
                        </div>
                        <div style="display:flex; align-items:center; gap:4px;">
                            <span style="font-size:11px; color:#64748b; white-space:nowrap;">To</span>
                            <input type="time"
                                   value="${req.endTime || ''}"
                                   onchange="updateEditRequirementTime(${serviceIndex}, ${reqIndex}, 'end', this.value)"
                                   style="width:120px;">
                        </div>
                        <button type="button" class="btn-remove-requirement"
    ...
    // Update catering type (Add modal)
    function updateCateringType(serviceIndex, value) {
        selectedServices[serviceIndex].cateringType = value;
        updateHiddenInput();
    }

    // Update catering type (Edit modal)
    function updateEditCateringType(serviceIndex, value) {
        editSelectedServices[serviceIndex].cateringType = value;
        updateEditHiddenInput();
    }
            ...
            <?php if (!empty($edit_event['service'])): ?>
                ...
                if (Array.isArray(serviceData)) {
                    editSelectedServices = serviceData.map(service => {
                        const subcategories = getRequirementsForService(service.service);

                        // Old format (requirements as string)
                        ...
                        // New format
                            ...
                            return {
                                ...
                                cateringType: service.cateringType || '',
                                cateringStartTime: service.cateringStartTime || '',
                                cateringEndTime: service.cateringEndTime || ''
                            };
                        }
                        // Fallback
                        else {
    ...
    function viewDetails(eventData) {
        ...
        if (!Array.isArray(services) || services.length === 0) {
            ...
            tbody.innerHTML = services.map((service, index) => {
                    // Build service name cell — catering type badge shown here
                    let serviceNameHTML = `<strong>${service.service}</strong>`;
                    if (isCatering(service.service)) {
                        const ct = service.cateringType || 'Not specified';
                        const cls = ct === 'Veg' ? 'veg' : ct === 'Non-Veg' ? 'nonveg' : ct === 'Both' ? 'both' : '';
                        const dot = cls ? `<span class="catering-dot ${cls}" style="width:10px;height:10px;border-radius:50%;display:inline-block;flex-shrink:0;"></span>` : '';
                        if (isCatering(service.service)) {
                            const ct = service.cateringType || 'Not specified';
                            const cls = ct === 'Veg' ? 'veg' : ct === 'Non-Veg' ? 'nonveg' : ct === 'Both' ? 'both' : '';
                            const dot = cls ? `<span class="catering-dot ${cls}" style="width:10px;height:10px;border-radius:50%;display:inline-block;flex-shrink:0;"></span>` : '';
                            const startTime = service.cateringStartTime ? formatTime(service.cateringStartTime) : '—';
                            const endTime = service.cateringEndTime ? formatTime(service.cateringEndTime) : '—';

                            serviceNameHTML = `
        <div style="display:flex; flex-direction:column; gap:6px;">
            <strong>${service.service}</strong>
            <span class="catering-type-badge ${cls}">${dot}${ct}</span>
        </div>`;
                        }
                    }

                    // Build requirements cell — normal for all services including catering
                    let requirementsHTML = '';
                    if (Array.isArray(service.requirements) && service.requirements.length > 0) {
                        const isCat = isCatering(service.service);
                        requirementsHTML = `
            <div class="view-requirements-list" style="gap:6px; width:100%;">
                ${service.requirements.map(req => {
                    let timeHTML = '';
                    if (isCat && (req.startTime || req.endTime)) {
                        const st = req.startTime ? formatTime(req.startTime) : '—';
                        const et = req.endTime   ? formatTime(req.endTime)   : '—';
                        timeHTML = `<span style="font-size:11px; color:#64748b; white-space:nowrap;">🕐 ${st} – ${et}</span>`;
                    }
                    return `<div class="view-requirement-item" style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                        <span class="view-requirement-type">${req.type || 'Not specified'}</span>
                        ${req.description ? `<span class="view-requirement-count">${req.description}</span>` : ''}
                        ${req.amount ? `<span style="display:inline-flex;align-items:center;padding:4px 10px;background:#dcfce7;color:#166534;border-radius:20px;font-size:11px;font-weight:700;">₹${parseFloat(req.amount).toLocaleString('en-IN')}</span>` : ''}
                        ${timeHTML}
                    </div>`;
                    }).join('')
            } </div>`;
    ...
    // Save catering time (Add modal)
    function updateCateringTime(serviceIndex, which, value) {
        if (which === 'start') {
            selectedServices[serviceIndex].cateringStartTime = value;
        } else {
            selectedServices[serviceIndex].cateringEndTime = value;
        }
        updateHiddenInput();
    }

    // Save catering time (Edit modal)
    function updateEditCateringTime(serviceIndex, which, value) {
        if (which === 'start') {
            editSelectedServices[serviceIndex].cateringStartTime = value;
        } else {
            editSelectedServices[serviceIndex].cateringEndTime = value;
        }
        updateEditHiddenInput();
    }

    // Update requirement amount (Add modal)
    function updateRequirementAmount(serviceIndex, reqIndex, value) {
        selectedServices[serviceIndex].requirements[reqIndex].amount = value;
        updateHiddenInput();
    }

    // Update requirement amount (Edit modal)
    function updateEditRequirementAmount(serviceIndex, reqIndex, value) {
        editSelectedServices[serviceIndex].requirements[reqIndex].amount = value;
        updateEditHiddenInput();
    }

    // Save per-requirement time (Add modal)
    function updateRequirementTime(serviceIndex, reqIndex, which, value) {
        if (which === 'start') {
            selectedServices[serviceIndex].requirements[reqIndex].startTime = value;
        } else {
            selectedServices[serviceIndex].requirements[reqIndex].endTime = value;
        }
        updateHiddenInput();
    }

    // Save per-requirement time (Edit modal)
    function updateEditRequirementTime(serviceIndex, reqIndex, which, value) {
        if (which === 'start') {
            editSelectedServices[serviceIndex].requirements[reqIndex].startTime = value;
        } else {
            editSelectedServices[serviceIndex].requirements[reqIndex].endTime = value;
        }
        updateEditHiddenInput();
    }
    ...
    function pastEventModal() {
        document.getElementById('pastEventsModal').classList.add('active');
        document.body.style.overflow = 'hidden';
    }

    function closePastEventsModal() {
        document.getElementById('pastEventsModal').classList.remove('active');
        document.body.style.overflow = 'auto';
    }

    document.getElementById('pastEventsModal').addEventListener('click', function(e) {
        if (e.target === this) closePastEventsModal();
    });

    function exportExcelPast() {
    const table = document.querySelector('#pastEventsModal table');
    if (!table) return;
    ...
    const filename = 'past_events_' + new Date().toLocaleDateString() + '.xls';
    const link = document.createElement('a');
    link.style.display = 'none';
    link.setAttribute('href', 'data:text/xls;charset=utf-8,' + encodeURIComponent(xls.join('\n')));
    link.setAttribute('download', filename);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
...
function printTablePast() {
    const header = document.createElement('div');
    header.id = 'print-past-header';
    header.innerHTML = `
        <div style="text-align:center; padding:4px 0 10px; border-bottom:2px solid #0ea5e9; margin-bottom:16px;">
            <h2 style="font-family:'Outfit',sans-serif; font-size:22px; font-weight:700; color:#1e293b; margin:0;">
                Past Event Details
    ...
    `;
    document.querySelector('#pastEventsModal .modal-body').prepend(header);
    ...
    setTimeout(() => {
        ...
        document.getElementById('print-past-header')?.remove();
    }, 100);
}

document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('pastSearchInput').addEventListener('keyup', function() {
        const searchValue = this.value.toLowerCase();
        const rows = document.querySelectorAll('#pastEventsModal table tbody tr');
